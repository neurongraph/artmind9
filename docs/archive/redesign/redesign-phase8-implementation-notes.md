...
- [neurongraph/artmind9#18](https://github.com/neurongraph/artmind9/issues/18) — **Finding A** (row 1): no pipeline stage exists to propose same-domain, same-class near-duplicate entities; `sameas propose` is a cross-domain adjudicator only. A real gap, not a tuning problem.
