...
9. **Temporal structured table:** a structured table (e.g. `vulnerable_customers.csv` or `complaints.csv`) re-ingested at least twice with `--refreshMode temporal --businessKey <key>`, with a believable field change between batches (e.g. a case's `reviewed_by`/`support_needed` changing, or a complaint's `status` progressing from `Open` to `Resolved`), so a benchmark question can test `artmind db timeline <table> --asOf <date>` against verified before/after state. Every structured table in this corpus is currently ingested in `replace` mode only (confirmed via `artmind db list --domain banking --compact`) — there is no SCD-2 history anywhere in it to query yet.
